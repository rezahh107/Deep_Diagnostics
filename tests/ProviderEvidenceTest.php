...
final class ProviderEvidenceTest extends TestCase {
    ...
    public function test_incident_order_first_boundary_and_degraded_meaning_are_preserved(): void {
        $service = $this->service(); self::assertTrue($service->importGppBundle($this->fixture)['ok']);
    ...
    public function test_absent_exact_ref_never_becomes_timing_correlation(): void {
        ...
        self::assertFalse($correlation['exact']); self::assertFalse($correlation['linked']);
        self::assertSame('exact_correlation_ref_absent', $correlation['reason']);
    }

    public function test_privacy_boundary_is_retained_and_hostile_unknown_fields_are_omitted(): void {
        $bundle = $this->bundle();
        ...
        $service = $this->service(); self::assertTrue($service->importGppBundle((string) wp_json_encode($bundle))['ok']);
        ...
        foreach ( ['CANARY-RAW-BODY-8841', 'private-person@example.test', 'CANARY-NATIONAL-ID-991122', 'CANARY-EXCEPTION-ARG-3388', 'submitted_value', 'exception'] as $forbidden ) self::assertStringNotContainsString($forbidden, $stored);
    }
    ...
    public function test_fake_direct_provider_proves_contract_is_generic(): void {
        add_filter(ProviderContract::REGISTRATION_FILTER, static function(array $providers): array {
            $providers[] = [
                'contract_version' => '1.0.0', 'provider_key' => 'fake_provider', 'name' => 'Fake Provider', 'provider_version' => '2.3.4', 'schema_version' => '1.0.0', 'capabilities' => ['health_snapshot'],
                'snapshot_callback' => static fn(): array => ['schema_version' => '1.0.0', 'generated_at_utc' => '2026-09-16T12:00:00+00:00', 'environment' => ['plugin_api' => '2.0.0'], 'components' => [['key' => 'delivery.sms', 'status' => 'ready']], 'unresolved' => [], 'incidents' => [], 'recent_success' => [], 'privacy_boundary' => ['message_bodies' => 'OMITTED'], 'claim_ceiling' => 'provider_declared_evidence_only'],
            ]; return $providers;
        });
        $service = $this->service(); self::assertTrue($service->captureDirect('fake_provider')['ok']);
    ...
    private function service(): ProviderEvidenceService { return new ProviderEvidenceService(new ProviderRegistry(), new ProviderEvidenceStore(static fn(): int => 1789550000)); }
    private function bundle(): array { $decoded = json_decode($this->fixture, true, 64, JSON_THROW_ON_ERROR); self::assertIsArray($decoded); return $decoded; }
}
