...
$connectionStatus = is_string($connection['status'] ?? null) ? $connection['status'] : 'unknown';
$currentNoticeClass = ! empty($unresolved) || in_array($connectionStatus, ['incompatible_provider_schema', 'evidence_store_error'], true)
    ? 'notice-warning'
    : 'notice-info';
...
$providerActionNotices = [
    ...
    'invalid_source_timestamp' => ['error', __('Import failed: the provider evidence does not contain a valid source timestamp, so DEEP cannot preserve the evidence chronology truthfully.', 'wp-deep-diagnostics')],
    ...
    'direct_snapshot_invalid' => ['error', __('The direct provider returned an invalid diagnostic snapshot.', 'wp-deep-diagnostics')],
...
$stateExplanations = [
    ...
    'unmapped' => __('GPP reports that this semantic meaning is not currently mapped to an authoritative source.', 'wp-deep-diagnostics'),
    'stale_source_missing' => __('GPP reports that the previously authoritative source is no longer present in the host form and the mapping needs repair in GPP.', 'wp-deep-diagnostics'),
    'ambiguous_needs_review' => __('GPP reports that this mapping needs an explicit administrator decision rather than an automatic guess.', 'wp-deep-diagnostics'),
    'evidence_not_proven' => __('GPP reports that the binding evidence is not proven yet.', 'wp-deep-diagnostics'),
    'UNKNOWN' => __('The provider explicitly reports this fact as unknown or unproven.', 'wp-deep-diagnostics'),
...
$eventResultExplanations = [
    'PASS' => __('This provider boundary was observed as passed.', 'wp-deep-diagnostics'),
    'FAIL' => __('The provider recorded a failure at this boundary.', 'wp-deep-diagnostics'),
    'SKIP' => __('The provider skipped this boundary; inspect the supplied reason and fallback.', 'wp-deep-diagnostics'),
    'NOT_APPLICABLE' => __('The provider marked this boundary as not applicable.', 'wp-deep-diagnostics'),
];
?>
<section class="wddtf-card" id="wddtf-gpp-diagnostics" aria-labelledby="wddtf-gpp-title">
    ...
    <div class="wddtf-columns">
        <div>
            ...
            <p><?php esc_html_e('It does not inspect GPP private wp_options or collect submitted values, names, national IDs, phone numbers, uploaded file names/contents, cookies, tokens, credentials, sensitive headers, raw bodies, absolute server paths, exception argument values, or raw form identifiers.', 'wp-deep-diagnostics'); ?></p>
        </div>
        <div>
    ...
    <div class="notice <?php echo esc_attr($currentNoticeClass); ?> inline" role="status"><p><strong><?php echo esc_html((string) ($interpretation['result'] ?? __('No provider interpretation is available.', 'wp-deep-diagnostics'))); ?></strong><br><?php echo esc_html((string) ($interpretation['meaning'] ?? '')); ?></p></div>

    <h3><?php esc_html_e('What this means', 'wp-deep-diagnostics'); ?></h3>
    <table class="widefat striped wddtf-kv-table"><tbody>
        <tr><th scope="row"><?php esc_html_e('Evidence source', 'wp-deep-diagnostics'); ?></th><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($snapshot['source']['mode'] ?? 'not available')); ?></code></td></tr>
        <tr><th scope="row"><?php esc_html_e('Provider current status', 'wp-deep-diagnostics'); ?></th><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($snapshot['current']['status'] ?? 'not available')); ?></code></td></tr>
        ...
        <tr><th scope="row"><?php esc_html_e('Provider / source schema', 'wp-deep-diagnostics'); ?></th><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) (($snapshot['provider']['schema_version'] ?? 'n/a') . ' / ' . ($snapshot['source']['schema_version'] ?? 'n/a'))); ?></code></td></tr>
    ...
    <?php else : ?>
        ...
        <?php foreach ( $unresolved as $fact ) : ?>
            <?php
            $rawState = is_string($fact['state'] ?? null) ? $fact['state'] : 'UNKNOWN';
            $rawStatus = is_string($fact['status'] ?? null) ? $fact['status'] : null;
            $factState = in_array($rawState, ['UNBOUND', 'NOT_PROVEN', 'UNKNOWN'], true) ? $rawState : ($rawStatus ?? $rawState);
            ?>
            <tr><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($fact['kind'] ?? 'unknown')); ?></code></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($fact['key'] ?? 'unknown')); ?></code></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html($factState); ?></code></td><td><?php echo esc_html($stateExplanations[$factState] ?? __('GPP marked this fact unresolved/unproven. Review it in GPP rather than repairing it in DEEP.', 'wp-deep-diagnostics')); ?></td></tr>
        <?php endforeach; ?>
        </tbody></table>
    <?php endif; ?>
    ...
    <?php if ( ! empty($incidents) ) : ?>
        ...
        <?php foreach ( $incidents as $incident ) : $first = is_array($incident['first_inconsistent_boundary'] ?? null) ? $incident['first_inconsistent_boundary'] : []; ?>
            <details class="wddtf-trace"><summary><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($incident['surface'] ?? 'provider')); ?></code> — <code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($incident['status'] ?? 'unknown')); ?></code> — <code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($incident['observed_at_utc'] ?? 'time unavailable')); ?></code></summary>
                ...
                <?php if ( ! empty($incident['fallback_note']) ) : ?><p><strong><?php esc_html_e('Fallback meaning:', 'wp-deep-diagnostics'); ?></strong> <?php echo esc_html((string) $incident['fallback_note']); ?></p><?php endif; ?>
                <div class="wddtf-table-scroll"><table class="widefat striped"><thead><tr><th>#</th><th><?php esc_html_e('Stage', 'wp-deep-diagnostics'); ?></th><th><?php esc_html_e('Result', 'wp-deep-diagnostics'); ?></th><th><?php esc_html_e('Plain-language result', 'wp-deep-diagnostics'); ?></th><th><?php esc_html_e('Reason', 'wp-deep-diagnostics'); ?></th><th><?php esc_html_e('Fallback', 'wp-deep-diagnostics'); ?></th></tr></thead><tbody><?php foreach ( $incident['events'] ?? [] as $event ) : $eventResult = (string) ($event['result'] ?? ''); ?><tr><td><?php echo esc_html((string) ($event['seq'] ?? '')); ?></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($event['stage'] ?? '')); ?></code></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html($eventResult); ?></code></td><td><?php echo esc_html($eventResultExplanations[$eventResult] ?? __('Provider result retained without additional interpretation.', 'wp-deep-diagnostics')); ?></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($event['reason_code'] ?? '')); ?></code></td><td><code class="wddtf-tech" dir="ltr"><?php echo esc_html((string) ($event['fallback'] ?? '')); ?></code></td></tr><?php endforeach; ?></tbody></table></div>
