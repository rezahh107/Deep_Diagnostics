...
final class ProviderEvidenceService {
    ...
    public function importGppBundle(string $raw): array {
        if ( '' === $raw ) return $this->failure('empty_import', __('The selected Support Bundle is empty.', 'wp-deep-diagnostics'));
        if ( strlen($raw) > ProviderContract::MAX_IMPORT_BYTES ) {
            return $this->failure('import_too_large', sprintf(__('The Support Bundle is larger than the %d KiB import limit.', 'wp-deep-diagnostics'), intdiv(ProviderContract::MAX_IMPORT_BYTES, 1024)));
        }
        try {
        ...
        if ( ! is_array($decoded) ) return $this->failure('invalid_json', __('The selected JSON does not contain a diagnostic bundle object.', 'wp-deep-diagnostics'));
    ...
    public function captureDirect(string $providerKey): array {
        ...
        if ( null === $registration ) return $this->failure('direct_provider_unavailable', __('No compatible direct diagnostic provider is registered for this provider key.', 'wp-deep-diagnostics'));
        try {
    ...
    public function diagnostics(string $providerKey, array $deepReport = []): array {
        ...
        foreach ( $discovery['errors'] as $error ) {
            if ( ($error['provider_key'] ?? null) === $providerKey ) {
        ...
        if ( is_array($snapshot['incidents'] ?? null) ) {
            foreach ( $snapshot['incidents'] as $incident ) {
                if ( is_array($incident) ) $incidents[] = $this->interpretIncident($incident, $deepReport);
            }
        }
    ...
    public function exportJson(string $providerKey, array $deepReport = []): ?string {
        ...
        if ( ! is_array($diagnostics['technical_evidence'] ?? null) ) return null;
        $export = [
            'export_schema_version' => '1.0.0',
            'exported_by' => ['plugin' => 'WP Deep Diagnostics', 'version' => defined('WDDTF_VERSION') ? WDDTF_VERSION : 'unknown'],
    ...
    private function connectionState(string $providerKey, ?array $snapshot, ?array $registration, ?string $registryError): array {
        if ( null !== $registryError ) return ['status' => 'incompatible_provider_schema', 'label' => __('Incompatible direct provider registration', 'wp-deep-diagnostics'), 'meaning' => __('A plugin attempted to register diagnostic evidence, but its provider contract is not compatible with this Deep Diagnostics build.', 'wp-deep-diagnostics')];
        if ( is_array($snapshot) && 'direct' === ($snapshot['source']['mode'] ?? null) ) return ['status' => 'direct_provider_connected', 'label' => __('Direct provider connected', 'wp-deep-diagnostics'), 'meaning' => __('The current evidence came directly from a compatible registered provider callback.', 'wp-deep-diagnostics')];
        if ( is_array($snapshot) ) return ['status' => 'support_bundle_loaded', 'label' => __('Compatible support bundle loaded', 'wp-deep-diagnostics'), 'meaning' => null !== $registration ? __('The current evidence came from an imported bundle. A compatible direct provider is also registered and can be refreshed explicitly.', 'wp-deep-diagnostics') : __('The current evidence came from an imported privacy-safe support bundle.', 'wp-deep-diagnostics')];
        if ( null !== $registration ) return ['status' => 'direct_provider_available_no_snapshot', 'label' => __('Direct provider available — no snapshot loaded yet', 'wp-deep-diagnostics'), 'meaning' => __('A compatible provider is registered, but Deep Diagnostics has not collected a snapshot. Use the explicit refresh action below.', 'wp-deep-diagnostics')];
        if ( 'gpp' === $providerKey && $this->gppDetected() ) return ['status' => 'provider_detected_no_data', 'label' => __('GPP detected but no provider data is available', 'wp-deep-diagnostics'), 'meaning' => __('GPP appears to be loaded, but it has not registered a compatible direct provider and no compatible Support Bundle has been imported.', 'wp-deep-diagnostics')];
        return ['status' => 'provider_not_detected', 'label' => 'gpp' === $providerKey ? __('GPP provider data not detected', 'wp-deep-diagnostics') : __('Provider data not detected', 'wp-deep-diagnostics'), 'meaning' => __('No compatible direct provider or imported evidence is currently available.', 'wp-deep-diagnostics')];
    }
    ...
    private function interpret(string $providerKey, ?array $snapshot): array {
        if ( null === $snapshot ) {
            return [
                ...
                'next_step' => 'gpp' === $providerKey ? __('Generate a Support Bundle in GPP and import it here, or refresh a compatible direct provider if GPP registers one.', 'wp-deep-diagnostics') : __('Load compatible provider evidence using one of the supported acquisition modes.', 'wp-deep-diagnostics'),
        ...
        if ( empty($unresolved) ) {
            ...
            $result = sprintf(_n('%d unresolved or unproven current fact is present in the provider snapshot.', '%d unresolved or unproven current facts are present in the provider snapshot.', count($unresolved), 'wp-deep-diagnostics'), count($unresolved));
        ...
        $proven = [];
        if ( is_string($sourceTime) && '' !== $sourceTime ) $proven[] = sprintf(__('The admitted snapshot represents provider evidence generated at %s.', 'wp-deep-diagnostics'), $sourceTime);
        $componentCount = count($snapshot['current']['components'] ?? []);
        $proven[] = sprintf(_n('%d active/current component was admitted.', '%d active/current components were admitted.', $componentCount, 'wp-deep-diagnostics'), $componentCount);
        if ( ! empty($incidents) ) $proven[] = sprintf(_n('%d historical provider incident is retained separately.', '%d historical provider incidents are retained separately.', count($incidents), 'wp-deep-diagnostics'), count($incidents));
        ...
        return [
            ...
            'next_step' => $this->nextStepForUnresolved($unresolved),
            'limitations' => [
    ...
    private function nextStepForUnresolved(array $unresolved): string {
        foreach ( $unresolved as $fact ) {
            ...
            if ( 'binding' === $kind && 'UNBOUND' === $state ) return __('At least one GPP binding is UNBOUND. Open GPP Mapping & Binding Health and explicitly map the semantic meaning there; DEEP will not repair it.', 'wp-deep-diagnostics');
            if ( 'runtime_claim' === $kind || 'NOT_PROVEN' === $state ) return __('At least one GPP condition is NOT_PROVEN. Review the corresponding mapping/readiness evidence in GPP and reproduce the relevant surface after GPP has established that proof.', 'wp-deep-diagnostics');
            if ( in_array($fact['status'] ?? null, ['stale_source_missing', 'ambiguous_needs_review'], true) ) return __('GPP reports a mapping that needs review. Use GPP Mapping & Binding Health to resolve the authoritative mapping before collecting another snapshot.', 'wp-deep-diagnostics');
        }
        return empty($unresolved) ? __('If the real behavior is still wrong, reproduce it and collect a fresh provider snapshot or incident so the missing boundary can be observed.', 'wp-deep-diagnostics') : __('Review the first unresolved provider fact in GPP, resolve it in the owning plugin, then collect a new snapshot to confirm the state changed.', 'wp-deep-diagnostics');
    }
    ...
    private function interpretIncident(array $incident, array $deepReport): array {
        ...
        return $incident + [
            ...
            'proves' => null === $first ? __('Only that the provider supplied this bounded incident chronology.', 'wp-deep-diagnostics') : sprintf(__('The provider sequence reached the recorded boundary %s and first deviated from PASS there.', 'wp-deep-diagnostics'), (string) $stage),
            ...
            'next_step' => $next,
            'fallback_note' => null !== $fallback ? __('The provider recorded a fallback at this boundary; DEEP treats it as provider evidence, not as proof of host failure.', 'wp-deep-diagnostics') : null,
    ...
    private function nextStepForStage(string $stage): string {
        if ( str_contains($stage, 'PROFILE') ) return __('Inspect the corresponding profile/surface activation and readiness in GPP, then collect a fresh snapshot after the owning GPP state is corrected.', 'wp-deep-diagnostics');
        if ( str_contains($stage, 'BINDING') ) return __('Inspect the corresponding mapping in GPP Mapping & Binding Health. Repair it in GPP, not in DEEP, then collect a fresh snapshot.', 'wp-deep-diagnostics');
        if ( str_contains($stage, 'HOST') ) return __('Inspect the host context/authorization evidence together with GPP’s own incident details. DEEP cannot decide from this provider event alone which host condition was absent.', 'wp-deep-diagnostics');
    ...
    private function correlate(?array $snapshot, array $deepReport): array {
        if ( null === $snapshot ) return ['exact' => false, 'linked' => false, 'reason' => 'provider_snapshot_unavailable', 'meaning' => __('No provider snapshot is available for correlation.', 'wp-deep-diagnostics')];
    ...
    private function correlateReference(mixed $reference, array $deepReport): array {
        if ( ! is_string($reference) || '' === $reference ) return ['exact' => false, 'linked' => false, 'reason' => 'exact_correlation_ref_absent', 'meaning' => __('No exact compatible correlation reference was supplied. Provider and DEEP evidence may be viewed together only as context; causal linkage is not proven.', 'wp-deep-diagnostics')];
        foreach ( $this->deepCorrelationReferences($deepReport) as $candidate ) {
            if ( hash_equals($candidate, $reference) ) return ['exact' => true, 'linked' => true, 'reason' => 'exact_reference_match', 'meaning' => __('The provider supplied an exact reference that matches retained DEEP evidence. Only that explicitly matched evidence is correlated.', 'wp-deep-diagnostics')];
        }
        return ['exact' => true, 'linked' => false, 'reason' => 'exact_reference_not_found', 'meaning' => __('The provider supplied an exact correlation reference, but no matching retained DEEP evidence is available. DEEP does not substitute timestamp proximity.', 'wp-deep-diagnostics')];
    }

    private function deepCorrelationReferences(array $report): array {
        $references = [];
        $cron = $report['layers']['cron']['qualification']['session_id'] ?? null;
        if ( is_string($cron) && '' !== $cron ) $references[] = $cron;
        $gravity = $report['layers']['gravity']['inbox_observation'] ?? [];
        $gravitySession = $gravity['session_id'] ?? null;
        if ( is_string($gravitySession) && '' !== $gravitySession ) $references[] = $gravitySession;
        if ( is_array($gravity['traces'] ?? null) ) {
            foreach ( $gravity['traces'] as $trace ) {
                $ref = is_array($trace) ? ($trace['trace_ref'] ?? null) : null;
                if ( is_string($ref) && '' !== $ref ) $references[] = $ref;
            }
        }
