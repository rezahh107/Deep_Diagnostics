...
final class Admin_Page {
    ...
    public function runCronQualification(): void {
        $this->authorize(
            self::CRON_ACTION,
            __('Cron qualification requires an explicit POST request.', 'wp-deep-diagnostics')
        );
    ...
    public function importProviderBundle(): void {
        $this->authorize(
            self::PROVIDER_IMPORT_ACTION,
            __('Provider evidence import requires an explicit POST request.', 'wp-deep-diagnostics')
        );

        $file = $_FILES['wddtf_provider_bundle'] ?? null;
        if ( ! is_array($file) || UPLOAD_ERR_OK !== ($file['error'] ?? null) ) {
            $this->redirectProviderAction('upload_error');
        }

        $declaredSize = isset($file['size']) && is_numeric($file['size']) ? (int) $file['size'] : 0;
        if ( $declaredSize <= 0 ) {
            $this->redirectProviderAction('empty_import');
        }
        if ( $declaredSize > ProviderContract::MAX_IMPORT_BYTES ) {
            $this->redirectProviderAction('import_too_large');
        }
        ...
        $raw = file_get_contents(
            ...
            null,
            0,
            ProviderContract::MAX_IMPORT_BYTES + 1
        );
        if ( false === $raw ) {
            $this->redirectProviderAction('upload_error');
        }
    ...
    public function refreshProvider(): void {
        $this->authorize(
            self::PROVIDER_REFRESH_ACTION,
            __('Direct provider refresh requires an explicit POST request.', 'wp-deep-diagnostics')
        );
    ...
    public function exportProviderEvidence(): void {
        $this->authorize(
            self::PROVIDER_EXPORT_ACTION,
            __('Provider evidence export requires an explicit POST request.', 'wp-deep-diagnostics')
        );
        ...
        if ( null === $json ) {
            wp_die(esc_html__('No compatible provider evidence is available to export.', 'wp-deep-diagnostics'));
        }
    ...
    public function render(): void {
        ...
        if ( isset($_GET['wddtf_provider_action']) && is_string($_GET['wddtf_provider_action']) ) {
            $providerAction = sanitize_key(wp_unslash($_GET['wddtf_provider_action']));
        }

        // Keep the existing admin template unchanged and compose the additive Provider
        // section immediately after its stable page heading. Both templates own their
        // output escaping; no provider evidence is parsed from the rendered HTML.
        ob_start();
        ...
        if ( false === $headingEnd ) {
            echo $providerHtml . $pageHtml; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped templates.
            return;
        }

        $headingEnd += strlen('</h1>');
        echo substr($pageHtml, 0, $headingEnd) . $providerHtml . substr($pageHtml, $headingEnd); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped templates.
    }

    private function redirectProviderAction(string $reason): never {
        wp_safe_redirect(
            add_query_arg(
                'wddtf_provider_action',
                sanitize_key($reason),
                admin_url('tools.php?page=' . self::SLUG . '#wddtf-gpp-diagnostics')
            )
        );
        exit;
    }
